...
function displayRunFinderResults(runs, searchCriteria = null, actFilter = null, levelFilter = null) {
    ...
    let criteriaSummaryText = '';
    if (searchCriteria) {
        const parts = [];
        if (searchCriteria.char && searchCriteria.char !== 'All') parts.push(`キャラ: ${searchCriteria.char}`);
        if (searchCriteria.act) parts.push(`Act: ${searchCriteria.act}`);
        if (searchCriteria.level) parts.push(`レベル: ${searchCriteria.level}`);
        if (searchCriteria.include.length > 0) parts.push(`含む: ${searchCriteria.include.join(', ')}`);
        if (searchCriteria.exclude.length > 0) parts.push(`除く: ${searchCriteria.exclude.join(', ')}`);
        if (searchCriteria.includeBosses.length > 0) parts.push(`ボス: ${searchCriteria.includeBosses.join(', ')}`);
        criteriaSummaryText = parts.join(' / ');
    }
